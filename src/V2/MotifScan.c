...
/* gcc  MotifScan.c safealloc.c fasta.c motif.c -o MotifScan.exe -lm -lJudy -lpthread */ 
...
/* Main functions */
int MotifScan(char *, Pattern *, Pvoid_t, Pvoid_t *, SEQUENCES *, int, FILE *);  /*  Scan the proteome with a list of motifs, and count their occurences in the Proteome        */

void printUsage() {
  printf("Usage: MotifEnrichment.exe -e <filepath> -f <filepath> -o <filepath> [-a <string> g]\n");
  printf("Arguments (* = required):\n");
  printf("* -e, --enumfile  <filepath>  Path to enumerated motifs from file\n");
  printf("* -f, --fastafile <filepath>  Path to FASTA formatted sequences file\n");
  printf("* -o, --output    <filepath>  Path to output file\n");
  printf("  -a, --seqtype   <string>    Sequence type (AA or NT)\n");
 ...
 */
int MotifScan(char * alphabet, Pattern * pattern, Pvoid_t k_Array, Pvoid_t *m_Array, SEQUENCES *sequences, int nseq, FILE * OUT) {
  int s = 0, i=0, pos = 0, nletters = 0, p1=0, p2=0;

  char first = alphabet[0]; 
  char last = alphabet[0];
  int *nmotifs = NULL;
  char *motif = NULL;
  char **motifs = NULL;
  clock_t start, end;
  ...
  Matrix masks = getMotifMask(pattern->len,pattern->xmin,pattern->xmax);
  nmotifs = safecalloc(0,1 * sizeof(int));
  start = clock();  
  ...
  if( nmotifs[0] > 0){
    printf("SCANNED %d MOTIFS in %3.1f seconds.\n",nmotifs[0],cpu_time_used);
    fprintf(OUT, "#SCANNED %d MOTIFS\n",nmotifs[0]);
    fprintf(OUT, "#TIME %3.1f SECONDS\n",cpu_time_used);
  }
...
int main (int argc,char **argv){
  ...
  int nseq = 0; int npos = 0; int i = 0;  int verbose=0; int opt;
  int nletters = 0;
  char * fasta  = NULL; char * letters = NULL;
  ...
  OptionValues options = { NULL, NULL, NULL, NULL, false , 0};

  const char* short_options = "e:f:o:a:vg";
  /* Define the long options array */
  const struct option long_options[] = {
  { "enumfile", required_argument, NULL, 'e' },
  { "fastafile", required_argument, NULL, 'f' },
  { "output", required_argument, NULL, 'o' },
  { "seqtype", optional_argument, NULL, 'a' },
  ...
  MotifScan(letters, motif, S_kArray,&P_mArray,sequences,nseq,OUT);
